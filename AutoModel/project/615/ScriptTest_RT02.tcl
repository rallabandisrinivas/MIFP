	...
	*showentitybymark 3 1 2
	# 网格划分
		...
		if {$mesh == $::Test::mesh2D} {
			...
			*interactiveremeshsurf 1 0.5 0 0 2 1 1
			set i 0
			foreach surfId $surfList {
				set params [hm_getsurfaceedges $surfId]
				set shape_type [llength [split $params]]
				catch {
					*set_meshfaceparams $i 2 0 0 0 1 0.5 1 1
					*automesh $i 2 0
				}
				catch {
					*set_meshfaceparams $i 5 0 0 0 1 0.5 1 1
					*automesh $i 5 0
				}
				catch {
					*set_meshfaceparams $i 3 0 0 0 1 0.5 1 1
					*automesh $i 3 0
				}
				incr i 1
				}
...
# 创建SurfSet	
	...
	*setvalue loadsteps id=$::Test::stepId STATUS=2 2287="YES"
	# 1425-分析类型(2：静态)，118-开始时间，119-总时间，1789-最小增量步，1790-最大增量
