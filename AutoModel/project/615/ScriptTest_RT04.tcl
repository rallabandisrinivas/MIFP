...
proc meshSet {partId} {
	...
	*clearmark components 1
	...
	foreach cID $compID {
		...
		if {$index == 2} {
			set cName3 [hm_getvalue comps id=$cID dataname=name]
			set solid3 $cID
		}
		if {$index == 3} {
			set cName4 [hm_getvalue comps id=$cID dataname=name]
			set solid4 $cID
		}
		incr index 1
	} 
	...
	# 几何处理
	*createmark solids 1 $solid2
	*createmark solids 2 $solid1 $solid3 $solid4
	...
	*createstringarray 3 "pars: post_cln fix_comp_bdr tet_clps='0.100000,1.000000,0' max_size='0,0,1.79769e+308'" \
		"tet: 355 1.3 -1 0 0.8 0 0" "2d: 1 0 4 2.5 0.5 30 1"
	*createmark solids 1 $solid1 $solid2 $solid3 $solid4
	*tetmesh solids 1 1 elements 0 -1 1 3
	puts \t网格划分：$cName1
	puts \t网格划分：$cName2
	puts \t网格划分：$cName3
